...
MathExpression::MathExpression(const MathExpression& e) : expression(e.getExpression()) {
}
...
MathExpression MathExpression::operator+=(char c) {
	return expression+=c;
}
...
bool MathExpression::isOpenParenthesis(char symbol) {
	return (symbol == '(');
}
bool MathExpression::isCloseParenthesis(char symbol) {
	return (symbol == ')');
}
