...
MathExpression RpnAlgorithm::conversionToAMachineReadableString() {
	for (int i = 0; i < inputString.getSize(); i++) {
		...
		if (inputString.isOperand(inputString[i])) {
			while (i < inputString.getSize() && inputString.isOperand(inputString[i])) {
				if (inputString[i] == ',')
				{
					outputString += '.';
					i++;
			...
			while (!stackWithOperators.empty() && inputString.precedence(stackWithOperators.top()) >= inputString.precedence(inputString[i]) && !inputString.isOpenParenthesis(stackWithOperators.top())) {
...
MathExpression RpnAlgorithm::getOutputString() {
	return outputString;
}
